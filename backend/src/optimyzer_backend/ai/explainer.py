...
from optimyzer_backend.ai.cache import (
    CacheService,
    CacheType,
...
# ============== Sprint 11 — PROMPT_VERSION constants ==============
#
# Bump version при изменении соответствующего system prompt — cached entries
# старой версии не будут найдены при lookup (новый cache_key включает version).
# Старые entries остаются в БД до cleanup (Phase G). Можно явно invalidate
# через CacheService.invalidate_by_version().
#
# История версий:
#   PLAN_MSSQL v1 (Sprint 11) — initial cache wrap для SYSTEM_PROMPT_EXPLAIN_MSSQL_PLAN
...
def _plan_cache_type_for(req: PlanExplainRequest) -> CacheType:
    """Map engine + plan_format → CacheType."""
    if req.engine == "postgres":
        return (
            CacheType.PLAN_PG_JSON
            if req.plan_format == "json"
            else CacheType.PLAN_PG_TEXT
        )
    # mssql
    return (
        CacheType.PLAN_MSSQL_TEXT
        if req.plan_format == "text"
        else CacheType.PLAN_MSSQL_XML
    )


def _plan_canonical_for(req: PlanExplainRequest) -> str:
    """Canonicalize plan content + AI-relevant context.

    Включаем sql_text + warnings + missing_indexes + antipatterns в composite,
    т.к. они меняют AI response. Configuration context специально НЕ включаем —
    он добавляет переменность (разные конфы → разные имена объектов в hotspots),
    но детерминированный AI response при одинаковом плане ВСЁ РАВНО полезен:
    юзер получит ту же hotspot структуру с placeholder именами таблиц.

    Trade-off documented в ADR-058.
    """
    cache_type = _plan_cache_type_for(req)
    if cache_type == CacheType.PLAN_MSSQL_XML:
        plan_canonical = canonicalize_plan_mssql_xml(req.plan_xml)
    elif cache_type == CacheType.PLAN_MSSQL_TEXT:
        plan_canonical = canonicalize_plan_mssql_text(req.plan_xml)
    elif cache_type == CacheType.PLAN_PG_JSON:
        plan_canonical = canonicalize_plan_pg_json(req.plan_xml)
    else:  # PLAN_PG_TEXT
        plan_canonical = canonicalize_plan_pg_text(req.plan_xml)

    # sql_text trim+whitespace normalize — мелкие отличия не должны рвать cache
    sql_canonical = re.sub(r"\s+", " ", req.sql_text).strip()

    # warnings + indexes + antipatterns — sorted JSON для детерминированности
    def _stable_json(items: list[dict]) -> str:
        return json.dumps(
            sorted(items, key=lambda w: json.dumps(w, sort_keys=True)),
            sort_keys=True,
            ensure_ascii=False,
        )
    ...
    return (
        f"sql={sql_canonical}"
        f"|plan={plan_canonical}"
        f"|warn={_stable_json(req.planview_warnings)}"
        f"|idx={_stable_json(req.missing_indexes)}"
        f"|ap={_stable_json(req.detected_antipatterns)}"
    )


def _prompt_version_for_plan(req: PlanExplainRequest) -> str:
    if req.engine == "postgres":
        return PROMPT_VERSION_PLAN_PG
    return PROMPT_VERSION_PLAN_MSSQL
...
async def _explain_query_uncached(req: ExplainRequest) -> ExplainResponse:
    """Sprint 11 — Internal: actual AI call без cache layer.

    Sprint 11 Phase C обернул `explain_query` cache wrapper'ом, оригинальная
    логика осталась здесь без изменений.
    """
...
def _query_canonical_for(req: ExplainRequest) -> str:
    """Canonical input для Query AI cache key."""
    canonical_sdbl = canonicalize_sdbl(req.query_sdbl)
    # Sort diagnostics by code для детерминированности (порядок не важен для AI)
    diagnostics_sorted = sorted(
        [d.model_dump() for d in req.diagnostics],
        key=lambda d: (d.get("code", ""), d.get("range_start_line", 0), d.get("range_start_char", 0)),
    )
    canonical_diagnostics = json.dumps(
        diagnostics_sorted, sort_keys=True, ensure_ascii=False
    )
    # configuration_context специально НЕ включаем (см. _plan_canonical_for trade-off)
    return f"sdbl={canonical_sdbl}|diag={canonical_diagnostics}"
...
async def explain_query(
    ...
    """Sprint 11 Phase C — cache wrapper над `_explain_query_uncached`.

    TTL=90 дней. Cache key учитывает canonical SDBL + sorted diagnostics +
    PROMPT_VERSION_QUERY + model. Bsl-LS diagnostics могут обновляться — TTL
    защищает от устаревших объяснений.

    Args:
        req: исходный request (с force_refresh флагом).
        cache_service: optional override для тестов.

    Raises:
        AiNotConfiguredError, AiExplainerError — пробрасываются из _uncached.
    """
    cache = cache_service if cache_service is not None else get_cache()
    canonical = _query_canonical_for(req)
    cache_key = cache.compute_key(
        canonical, CacheType.QUERY, PROMPT_VERSION_QUERY, settings.ai_model_default
    )
    ...
    if not req.force_refresh:
        ...
        if cached_entry is not None:
            ...
            if cached is not None:
                logger.info(
                    "query AI cache hit key=%s.. age=%ds",
                    cache_key[:12],
                    cached_entry.age_seconds(),
                )
                response = ExplainResponse(**cached)
    ...
    # AI call
    response = await _explain_query_uncached(req)

    # Cache store (90 days TTL)
    ...
    await cache.aset(
        ...
        prompt_version=PROMPT_VERSION_QUERY,
...
async def explain_plan_query(
    ...
    Sprint 11 Phase B: cache wrapper над дочерними функциями.
    Cache key учитывает plan (с canonicalize), sql_text, warnings, indexes,
    antipatterns, engine, prompt_version, model. Кешируется forever (план
    детерминирован — его объяснение детерминировано).

    Args:
        req: исходный request.
        cache_service: optional override для тестов. Production использует global singleton.
    ...
    cache = cache_service if cache_service is not None else get_cache()
    cache_type = _plan_cache_type_for(req)
    canonical = _plan_canonical_for(req)
    prompt_version = _prompt_version_for_plan(req)
    cache_key = cache.compute_key(
        canonical, cache_type, prompt_version, settings.ai_model_default
    )
    ...
    if not req.force_refresh:
        ...
        if cached_entry is not None:
            ...
            if cached is not None:
                logger.info(
                    "plan AI cache hit type=%s key=%s.. age=%ds",
                    cache_type.value,
                    cache_key[:12],
                    cached_entry.age_seconds(),
                )
                response = PlanExplainResponse(**cached)
                response.was_cached = True
                response.cache_age_seconds = cached_entry.age_seconds()
                response.cache_key = cache_key
                return response
    ...
    # AI call
    if req.engine == "postgres":
        response = await explain_pg_plan(req)
    else:
        response = await explain_mssql_plan(req)

    # Cache store (forever — план детерминирован)
    response_dict = response.model_dump()
    # Не кешируем cache metadata (она вычисляется при lookup)
    response_dict.pop("was_cached", None)
    response_dict.pop("cache_age_seconds", None)
    response_dict.pop("cache_key", None)
    await cache.aset(
...
async def _generate_logcfg_uncached(req: LogcfgGenerateRequest) -> LogcfgGenerateResponse:
    """Sprint 11 — Internal: actual Haiku call без cache layer."""
    if not settings.anthropic_api_key:
        raise AiNotConfiguredError(
...
def _logcfg_canonical_for(req: LogcfgGenerateRequest) -> str:
    """Canonical input для Logcfg AI cache key."""
    canonical_desc = canonicalize_logcfg_description(req.problem_description)
    return (
        f"desc={canonical_desc}"
        f"|platform={req.platform_version or 'none'}"
        f"|dbms={req.dbms or 'unknown'}"
    )


async def generate_logcfg(
    req: LogcfgGenerateRequest,
    cache_service: Optional[CacheService] = None,
) -> LogcfgGenerateResponse:
    """Sprint 11 Phase C — cache wrapper над `_generate_logcfg_uncached`.

    TTL=30 дней. AI prompt и шаблоны могут эволюционировать — TTL защищает
    от устаревших recommendations.
    ...
    """
    cache = cache_service if cache_service is not None else get_cache()
    canonical = _logcfg_canonical_for(req)
    cache_key = cache.compute_key(
        canonical, CacheType.LOGCFG, PROMPT_VERSION_LOGCFG, AI_MODEL_LOGCFG
    )
    ...
    if not req.force_refresh:
        ...
        if cached_entry is not None:
            ...
            if cached is not None:
                logger.info(
                    "logcfg AI cache hit key=%s.. age=%ds",
                    cache_key[:12],
                    cached_entry.age_seconds(),
                )
                response = LogcfgGenerateResponse(**cached)
                response.was_cached = True
                response.cache_age_seconds = cached_entry.age_seconds()
                response.cache_key = cache_key
                return response
    ...
    # AI call
    response = await _generate_logcfg_uncached(req)

    # Cache store (30 days TTL)
    response_dict = response.model_dump()
    response_dict.pop("was_cached", None)
    response_dict.pop("cache_age_seconds", None)
    response_dict.pop("cache_key", None)
