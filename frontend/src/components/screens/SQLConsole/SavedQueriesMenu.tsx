...
export function SavedQueriesMenu({ currentQuery, onLoadQuery }: Props) {
  ...
  const handleSave = useCallback(async () => {
    const name = window.prompt(`${t.sql.actions.save}: ${t.sql.saved.newQuery}?`);
  ...
  return (
    <div className={styles.saved_menu}>
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        title={t.sql.saved.label}
      >
        <Icon name="Bookmark" size={11} color="var(--o-text-3)" />
        {t.sql.saved.label}
        <Icon name="ChevronDown" size={10} color="var(--o-text-3)" />
      </button>
      <button className={styles.template_btn} onClick={handleSave} title={t.sql.actions.save}>
      ...
      {open && (
        <div className={styles.saved_dropdown}>
          {items.length === 0 ? (
            <div className={styles.saved_empty}>{t.sql.saved.empty}</div>
          ) : (
            items.map((it) => (
