...
## ADR-062 — BYOK: ключ Anthropic пользователя, сервер убран из AI-пути

**Status:** Accepted (2026-07-16). **Отменяет ADR-057** (asymmetric AI flow через сервер).

**Context.** По ADR-057 AI шёл фронт → наш сервер → Anthropic: ключ хранился у нас, ответы кэшировались на сервере. Отсюда тянулись биллинг, квоты, kill-switch, rate limiting (ADR-055) — вся эта машинерия существовала только чтобы ограничивать расход НАШЕГО ключа.

Вскрылись два факта:
1. **Эндпоинты `/v1/ai/*` были открыты анонимно** — единственной проверкой был kill-switch. Любой, кто знал адрес, мог тратить наш ключ (утечки самого ключа не было — он не покидал сервер). Закрыто авторизацией, но это лечило симптом.
2. **Сервер никогда не был задеплоен.** `api.optimyzer.pro` не резолвится ни одним DNS в мире (проверено глобально). То есть AI у пользователей **не работал вообще, ни разу** — приложение стучалось в несуществующий домен.

Модель «наш ключ» также противоречит цели продукта: она делает себестоимость пропорциональной числу пользователей и требует держать инфраструктуру.

**Decision.** Ключ Anthropic принадлежит **пользователю** и хранится только на его машине; вызовы идут напрямую с его компьютера в Anthropic. Наш сервер в AI-пути не участвует вообще.

- Оркестратор перенесён из `server/services/ai_explainer.py` в sidecar как есть (`optimyzer_backend/ai/`), чтобы поведение AI не разъехалось; переписаны только импорты, обращения к `settings.*` сохранены через шим `ai/config.py`.
- Ключ — в `metadata.sqlite` (APPDATA), наружу только маской. RPC: `ai_settings_*`, `ai_*`.
- Семантика трёх состояний ключа: `None` — не задан → откат на ENV (только dev); `""` — явно удалён → AI выключен **без** отката на ENV; `"sk-…"` — ключ пользователя, приоритетнее ENV. Иначе «удалил ключ» тихо означало бы «использую чужой ключ из окружения».
- Кэш AI переехал в APPDATA (рядом с exe писать нельзя — Program Files только на чтение).

**Consequences.**
+ Себестоимость пользователя — ноль; продажи не увеличивают расходы (ключевое для коммерческой модели).
+ Сервер, биллинг AI, квоты, kill-switch и rate limiting для AI больше не нужны — целый пласт кода и инфраструктуры выключается из критического пути.
+ AI наконец работает: раньше он был мёртв у всех из-за отсутствия сервера.
+ Приватность: запросы пользователя не проходят через нас.
− Порог входа: пользователю нужен свой ключ Anthropic (митигируется вкладкой «AI» с прямой ссылкой на console.anthropic.com).
− Оплачивает вызовы пользователь; это честно проговорено в UI.
− Код AI продублирован с `server/` (сервер остаётся для лицензий/авторизации).

**Серверные AI-эндпоинты, квоты, kill-switch и rate limiting НЕ удаляются — сохраняются намеренно.** Это законсервированная опция: позже возможен платный managed-AI (мы поднимаем сервер и продаём AI на нашем ключе) как апселл для тех, кому неудобно заводить свой ключ Anthropic — а это единственный минус BYOK. Сервер не задеплоен, поэтому код спит и ничего не стоит; удалить его = выбросить готовый путь к выручке ради косметики. Если код мешает (тесты, линтер) — чинить, но не удалять. Решение владельца, 2026-07-16.
...
## ADR-058 — Sidecar resilience: лог, авто-перезапуск, лимит памяти

**Status:** Accepted (2026-07-11)

**Context.** Пользователь (Инфостарт) при загрузке ТЖ с сетевой шары получал `Ошибка RPC: Идет закрытие канала. (os error 232)`. Разбор: `os error 232` = Windows `ERROR_NO_DATA` на записи в stdin sidecar'а (`sidecar.rs`) — значит Python-процесс **аварийно завершился**. Обычные ошибки перехватываются (dispatcher `except Exception` → JSON-RPC error; ingest-worker → `status='error'`), поэтому 232 = именно жёсткое падение (нативный segfault DuckDB/pyarrow либо исчерпание памяти при большом ТЖ). Усугублялось тем, что: (1) stderr sidecar'а шёл в `Stdio::inherit()` оконного приложения → лог падения терялся; (2) не было ни детекта смерти, ни перезапуска — любой следующий RPC утыкался в мёртвый pipe → приложение «кирпич» до перезапуска; (3) DuckDB по умолчанию берёт до 80% RAM, что вместе с WebView2/парсером/ОС давало OOM.

**Decision.** Три уровня защиты:
1. **Диагностируемость.** stderr sidecar'а пишется в файл-лог `%APPDATA%/1c-optimyzer/logs/backend.log` (+ Tauri-команда `backend_log_path`). Любое будущее падение теперь видно.
2. **Устойчивость (`sidecar.rs`).** При закрытии stdout (процесс умер) ожидающие запросы будятся понятной ошибкой (не зависают), sidecar автоматически перезапускается. `request()` при обрыве «до записи» прозрачно повторяет запрос на свежем процессе (пользователь ошибку не видит). Идемпотентность перезапуска — через счётчик поколений (`generation`); anti-storm — не более 5 перезапусков за 30 с (защита от креша-на-старте).
3. **Профилактика OOM (`duckdb_store.py`).** `memory_limit` = 60% физической RAM (env `OPTIMYZER_DUCKDB_MEMORY_LIMIT` переопределяет), `temp_directory` = локальный каталог → тяжёлые запросы спиллят на диск, а не роняют процесс.

**Auto-reattach.** После перезапуска `sidecar.rs` эмитит Tauri-событие `sidecar-restarted`; фронтенд (`App.tsx` через `onSidecarRestarted`) переоткрывает загруженный архив (`open_stored_archive` — `.duckdb` + SQLite-история на диске переживают крах), так что экраны продолжают работать без ручной перезагрузки папки. Если архив не завершил загрузку до краха (нет в истории) — просим перезагрузить папку.

**Consequences.** + Пользователь больше не видит сырой `os error 232` — либо прозрачное восстановление, либо понятное «перезапущен, повторите». + Приложение не превращается в «кирпич»; загруженный архив авто-восстанавливается. + Причина любого падения теперь в логе — корневой нативный сбой можно добить прицельно. − Узкая доброкачественная гонка: конкурентный запрос ровно в момент respawn может получить сообщение о перезапуске вместо ответа (повтор решает; ни зависания, ни потери данных). − `memory_limit` 60% может чуть замедлить очень тяжёлые запросы (спилл на диск) — приемлемо ради стабильности.
